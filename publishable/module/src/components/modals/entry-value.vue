...
async function submitModalForm(values) {
  ...
  const url = `${URL_PATH}/entries/group/${form.language_group_id}/key/${form.key_id}/language/${form.language_id}`
...
const language_title = computed(() => {
  const foundItem = Object.values(toRaw(modalStore.data.languages)).find(
    (el) => el.id === parseInt(modalStore.data.language_id),
  )
  if (foundItem) {
...
<template>
  ...
  >
    ...
    <div class="row mb-1 align-items-center">
      <div class="col-4">{{ t('entries.form.language') }}</div>
    ...
    <!-- ключ -->
    <div class="row mb-1 align-items-center">
      <div class="col-4">{{ t('entries.form.key') }}</div>
      <div class="col-8 py-2 ps-4">
        <strong>{{ modalStore.data.key }}</strong>
      </div>
    </div>
