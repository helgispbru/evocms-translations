...
async function submitModalForm(values) {
  ...
  if (modalStore.data.action == 'edit' && Number(modalStore.data.id) == 0) {
    error.value = t('groups.form.error.noid')
  ...
  switch (form.action) {
    ...
    case 'edit':
      method = 'PUT'
      url = `${URL_PATH}/groups/${form.id}`
      break
    default:
      error.value = t('groups.form.error.noaction')
  ...
  >
    ...
    <div class="row mb-1 align-items-center">
      <label
        ...
        for="validationGroupCode"
        >{{ t('groups.form.code') }}</label
      >
      <div class="col-8">
        <Field
          name="code"
          ...
          type="text"
          autocomplete="off"
          id="validationGroupCode"
          aria-describedby="validationGroupCodeFeedback"
        />
        <ErrorMessage
          name="code"
          as="div"
          class="invalid-feedback"
          id="validationGroupCodeFeedback"
        />
      </div>
    </div>
    ...
    <div class="row mb-1 align-items-center">
      <label
        ...
        for="validationGroupTitle"
        >{{ t('groups.form.title') }}</label
      >
      <div class="col-8">
        <Field
          name="title"
          ...
          type="text"
          autocomplete="off"
          id="validationGroupTitle"
          aria-describedby="validationGroupTitleFeedback"
        />
        <ErrorMessage
          name="title"
          as="div"
          class="invalid-feedback"
          id="validationGroupTitleFeedback"
        />
      </div>
    </div>
