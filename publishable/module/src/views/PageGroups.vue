...
import { onEditGroup, onDeleteGroup } from '@/composable/useBsModal'
...
const columns = computed(() => [
  { data: 'code', title: t('groups.cols.code'), orderable: true },
  { data: 'title', title: t('groups.cols.title'), orderable: true },
])
...
async function rowEdit(obj) {
  if (await onEditGroup(t, obj)) {
    $toast.success(t('groups.toast.edited'), TOAST_OPTIONS)
...
async function rowDelete(obj) {
  if (await onDeleteGroup(t, obj, $toast)) {
    $toast.success(t('groups.toast.deleted'), TOAST_OPTIONS)
...
<template>
  <div class="tab-content page-groups container-fluid">
    <h1 class="pt-3">{{ t('groups.title') }}</h1>
    <DataTable
      ref="dataTable"
      :key="groupsStore.tableKey"
      :columns="columns"
      :options="options"
      class="table"
    >
