...
**Важно!** После окончания работы сделать экспорт из базы в языковые файлы.

При экспорте данные из базы записываются в файлы `/core/custom/lang`, содержимое файлов полностью заменяется (если нужно - делайте резервные копии!). Удалённые в базе группы и языки при экспорте удаляются из файлов.
...
**После** окончания работы нужно экспортировать всё в файлы (кнопка доступна справа сверху). Важно делать это каждый раз, так как только файлы являются достоверным источником данных, а таблица в базе нужна лишь для удобной работы и служит для временного хранения строк переводов. При экспорте создаются папки для языков и файлы для групп с переводами. Если язык или группа были удалены из базы, то они также будут удалены из файлов (делайте резервные копии!).
...
Исходный код фронта модуля находится в `publishable/module`, написан с помощью `vuejs v3`.

Код для импорта и экспорта частично заимствован у [WeDesignIt/laravel-translations-import](https://github.com/WeDesignIt/laravel-translations-import), спасибо ему!
